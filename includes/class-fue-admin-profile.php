    ...
    public function add_api_key_field( $user ) {
        ...
        if ( current_user_can( 'edit_user', $user->ID ) ) {
            ...
            <table class="form-table">
                <tbody>
                <tr>
                    <th><label for="fue_generate_api_key"><?php _e( 'Follow-Up Emails API Keys', 'follow_up_emails' ); ?></label></th>
                    <td>
                        <?php if ( empty( $user->fue_api_consumer_key ) ) : ?>
                            <input name="fue_generate_api_key" type="checkbox" id="fue_generate_api_key" value="0" />
                            <span class="description"><?php _e( 'Generate API Key', 'follow_up_emails' ); ?></span>
                        <?php else : ?>
                            <strong><?php _e( 'Consumer Key:', 'follow_up_emails' ); ?>&nbsp;</strong><code id="fue_api_consumer_key"><?php echo $user->fue_api_consumer_key ?></code><br/>
                            <strong><?php _e( 'Consumer Secret:', 'follow_up_emails' ); ?>&nbsp;</strong><code id="fue_api_consumer_secret"><?php echo $user->fue_api_consumer_secret; ?></code><br/>
                            <strong><?php _e( 'Permissions:', 'follow_up_emails' ); ?>&nbsp;</strong><span id="fue_api_key_permissions"><select name="fue_api_key_permissions" id="fue_api_key_permissions"><?php
                                    foreach ( $permissions as $permission_key => $permission_name ) { echo '<option value="' . esc_attr( $permission_key ) . '" '.selected($permission_key, $user->fue_api_key_permissions, false).'>'.esc_html( $permission_name ) . '</option>';} ?>
                                </select></span><br/>
                            <input name="fue_generate_api_key" type="checkbox" id="fue_generate_api_key" value="0" />
                            <span class="description"><?php _e( 'Revoke API Key', 'follow_up_emails' ); ?></span>
                        <?php endif; ?>
                    </td>
                </tr>
                </tbody>
