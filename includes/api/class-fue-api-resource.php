...
class FUE_API_Resource {

	/** @var FUE_API_Server the API server */
	protected $server;
	...
	public function __construct( FUE_API_Server $server ) {
		...
		add_filter( 'fue_api_endpoints', array( $this, 'register_routes' ) );

		// maybe add meta to top-level resource responses
		foreach ( array( 'emails', 'queue', 'reports', 'campaigns' ) as $resource ) {
			add_filter( "fue_api_{$resource}_response", array( $this, 'maybe_add_meta' ), 15, 2 );
		}

		$response_names = array( 'email', 'queue', 'report' );
		...
		foreach ( $response_names as $name ) {
			 ...
			 */
			add_filter( "fue_api_{$name}_response", array( $this, 'filter_response_fields' ), 20, 3 );
		}
	}
	...
	protected function validate_request( $id, $type, $context ) {
		...
		// validate ID
		if ( empty( $id ) )
			return new WP_Error( "fue_api_invalid_{$resource_name}_id", sprintf( __( 'Invalid %s ID', 'follow_up_emails' ), $type ), array( 'status' => 404 ) );
        ...
        // validate post type
        if ( $type !== $post_type )
            return new WP_Error( "fue_api_invalid_{$resource_name}", sprintf( __( 'Invalid %s', 'follow_up_emails' ), $resource_name ), array( 'status' => 404 ) );
        ...
        switch ( $context ) {
            ...
            case 'read':
                if ( ! $this->is_readable( $post ) )
                    return new WP_Error( "fue_api_user_cannot_read_{$resource_name}", sprintf( __( 'You do not have permission to read this %s', 'follow_up_emails' ), $resource_name ), array( 'status' => 401 ) );
                break;

            case 'edit':
                if ( ! $this->is_editable( $post ) )
                    return new WP_Error( "fue_api_user_cannot_edit_{$resource_name}", sprintf( __( 'You do not have permission to edit this %s', 'follow_up_emails' ), $resource_name ), array( 'status' => 401 ) );
                break;

            case 'delete':
                if ( ! $this->is_deletable( $post ) )
                    return new WP_Error( "fue_api_user_cannot_delete_{$resource_name}", sprintf( __( 'You do not have permission to delete this %s', 'follow_up_emails' ), $resource_name ), array( 'status' => 401 ) );
                break;
        }
	...
	protected function merge_query_args( $base_args, $request_args ) {
		...
		$args = apply_filters( 'fue_api_query_args', $args, $request_args );
	 ...
	 * @param object $resource the resource object (e.g FUE_Email)
	 * @return mixed
	 */
	...
	protected function delete( $id, $type, $force = false ) {
        ...
        if ( ! $result )
            return new WP_Error( "fue_api_cannot_delete_{$resource_name}", sprintf( __( 'This %s cannot be deleted', 'follow_up_emails' ), $resource_name ), array( 'status' => 500 ) );

        if ( $force ) {
            return array( 'message' => sprintf( __( 'Permanently deleted %s', 'follow_up_emails' ), $resource_name ) );

        } else {

            $this->server->send_status( '202' );

            return array( 'message' => sprintf( __( 'Deleted %s', 'follow_up_emails' ), $resource_name ) );
        }
