...
class FUE_API_Server {
	...
	public function __construct( $path ) {
		...
		$handler_class = apply_filters( 'fue_api_default_response_handler', 'FUE_API_JSON_Handler', $this->path, $this );
	 ...
	 *
	 * @since 4.1
	 * @uses FUE_API_Server::dispatch()
	 */
	public function serve_request() {

		do_action( 'fue_api_server_before_serve', $this );

		$this->header( 'Content-Type', $this->handler->get_content_type(), true );

		// the API is enabled by default
		if ( ! apply_filters( 'fue_api_enabled', true, $this ) || ( 'no' === get_option( 'fue_api_enabled', 'yes' ) ) ) {

			$this->send_status( 404 );

			echo $this->handler->generate_response( array( 'errors' => array( 'code' => 'fue_api_disabled', 'message' => 'The Follow-Up Emails API is disabled on this site' ) ) );
		...
		$served = apply_filters( 'fue_api_serve_request', false, $result, $this );
	...
	public function get_routes() {
		...
		$endpoints = apply_filters( 'fue_api_endpoints', $endpoints );
	...
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			default:
				return new WP_Error( 'fue_api_unsupported_method', __( 'Unsupported request method', 'follow_up_emails' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) )
					return new WP_Error( 'fue_api_invalid_handler', __( 'The handler for the route is invalid', 'follow_up_emails' ), array( 'status' => 500 ) );
				...
				$args = apply_filters( 'fue_api_dispatch_args', $args, $callback );
		...
		return new WP_Error( 'fue_api_no_route', __( 'No route was found matching the URL and request method', 'follow_up_emails' ), array( 'status' => 404 ) );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			...
			elseif ( $param->isDefaultValueAvailable() ) {
				...
				return new WP_Error( 'fue_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'follow_up_emails' ), $param->getName() ), array( 'status' => 400 ) );
			}
		}
	 ...
	 *
	 * @since 4.1
	 * @param WP_Query|WP_User_Query|Array $query
	 */
	public function add_pagination_headers( $query ) {
        ...
        // Assoc Array
            ...
            $page        = $query['page'];
            $single      = (!empty( $query['single'] ) ) ? $query['single'] : false;
            $total       = $query['total'];
            $total_pages = $query['total_pages'];
        }
		...
		$this->header( 'X-FUE-Total', $total );
		$this->header( 'X-FUE-TotalPages', $total_pages );

		do_action( 'fue_api_pagination_headers', $this, $query );
	}
	...
	public function format_datetime( $timestamp, $convert_to_utc = false ) {
		...
		if ( $convert_to_utc ) {
			$timezone = new DateTimeZone( fue_timezone_string() );
		} else {
