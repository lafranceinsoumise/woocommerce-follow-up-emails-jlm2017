<?php
/**
 * FUE API Authentication Class based on WC_API_Authentication
 *
 * @author      WooThemes
 * @category    API
...
class FUE_API_Authentication {

	/**
	 * Setup class
	 *
	 * @since 4.1
	 * @return FUE_API_Authentication
	 */
	public function __construct() {

		// to disable authentication, hook into this filter at a later priority and return a valid WP_User
		add_filter( 'fue_api_check_authentication', array( $this, 'authenticate' ), 0 );
	}
	...
	private function perform_ssl_authentication() {
		...
		if ( ! empty( $_SERVER['PHP_AUTH_USER'] ) ) {
			...
			throw new Exception( __( 'Consumer Key is missing', 'follow_up_emails' ), 404 );
		}
		...
		if ( ! empty( $_SERVER['PHP_AUTH_PW'] ) ) {
			...
			throw new Exception( __( 'Consumer Secret is missing', 'follow_up_emails' ), 404 );
		}

		$user = $this->get_user_by_consumer_key( $consumer_key );

		if ( ! $this->is_consumer_secret_valid( $user, $consumer_secret ) ) {
			throw new Exception( __( 'Consumer Secret is invalid', 'follow_up_emails' ), 401 );
		}
	...
	private function perform_oauth_authentication() {
		...
		foreach ( $param_names as $param_name ) {
			...
			if ( empty( $params[ $param_name ] ) )
				throw new Exception( sprintf( __( '%s parameter is missing', 'follow_up_emails' ), $param_name ), 404 );
		}
     ...
     * Matches against FUE first, then checks against WC consumer key.
	 *
	 * @since 4.1
	 * @param WP_User $user
	...
	private function is_consumer_secret_valid( WP_User $user, $consumer_secret ) {
        ...
        if ( $user->fue_api_consumer_secret === $consumer_secret ) {
            $valid = true;
        } elseif ( $user->woocommerce_api_consumer_secret === $consumer_secret ) {
            $valid = true;
        }
	...
	private function check_oauth_timestamp_and_nonce( $user, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) ||  ( $timestamp > time() + $valid_window ) )
			throw new Exception( __( 'Invalid timestamp', 'follow_up_emails' ) );

		$used_nonces = $user->fue_api_nonces;

		if ( empty( $used_nonces ) )
			$used_nonces = array();

		if ( in_array( $nonce, $used_nonces ) )
			throw new Exception( __( 'Invalid nonce - nonce has already been used', 'follow_up_emails' ), 401 );
		...
		update_user_meta( $user->ID, 'fue_api_nonces', $used_nonces );
	}
