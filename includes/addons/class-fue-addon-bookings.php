...
class FUE_Addon_Bookings {
    ...
    public function email_variables_list( $email ) {
        ...
        <li class="var hideable var_wc_bookings var_wc_bookings_item_quantity"><strong>{item_quantity}</strong> <img class="help_tip" title="<?php _e('The quantity of the purchased item.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
        <li class="var hideable var_wc_bookings"><strong>{booking_start}</strong> <img class="help_tip" title="<?php _e('The start date of the booked product or service', 'follow_up_emails'); ?>" src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" width="16" height="16" /></li>
        <li class="var hideable var_wc_bookings"><strong>{booking_end}</strong> <img class="help_tip" title="<?php _e('The end date of the booked product or service', 'follow_up_emails'); ?>" src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" width="16" height="16" /></li>
                ...
                foreach ( $booking_order->get_items() as $item_id => $item ) {
                    ...
                    if ( $booking_product->id == $product_id ) {
                        ...
                        if ( isset( $item['Duration'] ) ) {
                            $variables['booking_duration'] = $item['Duration'];
                        }
                        break;
                    }
                }
    ...
    public function maybe_trigger_status_update( $post_id ) {
        if ( !empty($_POST['post_type']) && $_POST['post_type'] != 'wc_booking' ) {
            return $post_id;
        }

        if ( empty( $_POST['_booking_status'] ) ) {
            return $post_id;
        }
