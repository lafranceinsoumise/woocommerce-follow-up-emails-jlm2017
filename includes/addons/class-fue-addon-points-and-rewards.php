...
class FUE_Addon_Points_And_Rewards {
    ...
    public function get_manual_email_recipients( $recipients, $post ) {
        ...
        if ( $send_type == 'points_rewards_over' || $send_type == 'points_rewards_under' ) {
            $points = absint( $_POST['points_rewards_points'] );
    ...
    public function email_variables_list( $email ) {
        ...
        ?>
        <li class="var hideable var_points_and_rewards"><strong>{current_points}</strong> <img class="help_tip" title="<?php _e('The current user\'s number of points', 'wc_followup_emails'); ?>" src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" width="16" height="16" /></li>
