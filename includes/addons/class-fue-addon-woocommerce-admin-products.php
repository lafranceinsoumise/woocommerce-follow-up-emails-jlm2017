...
class FUE_Addon_Woocommerce_Admin_Products {
    ...
    public function add_product_data_panel() {
        ...
        <div id="follow_ups_product_data" class="panel woocommerce_options_panel">
            ...
            <div class="options_group">
                <p><?php _e('Add purchaser of this product to the following list(s)', 'follow_up_emails'); ?></p>
                <p class="form-field fue_lists_field">
                    ...
                    if ( !empty( $lists ) ) {
                        foreach ( $lists as $list ) {
                            echo '
                                <label>
                                    <input type="checkbox" name="fue_lists[]" value="'. $list['id'] .'" '. checked( true, in_array( $list['id'], $product_lists ), false ) .' />
                                    '. $list['list_name'] .'
                                </label>
                            <br/>';
                        }
            ...
            <div class="options_group">
                <p class="form-field">
                    <label for="new_fue_list"><?php _e('New List', 'follow_up_emails'); ?></label>
                    <input type="text" class="short" id="new_fue_list" style="width: 40%;" />
                    <input type="button" id="add_new_fue_list" class="button add-new-fue-list" value="<?php _e('Add', 'follow_up_emails'); ?>" />
                </p>
            </div>
    ...
    public function save_lists( $post_id, $post ) {
        if ( isset( $_POST['fue_lists'] ) ) {
            $lists = array_map( 'absint', $_POST['fue_lists'] );
