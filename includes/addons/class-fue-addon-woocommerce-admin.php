...
class FUE_Addon_Woocommerce_Admin {
    ...
    public function order_import_check() {
        if (
            ...
            <div id="message" class="updated">
                <p><strong><?php _e( 'Initial order scanning is required to accurately send conditional emails', 'follow_up_emails' ); ?></strong></p>
                <p class="submit">
    ...
    public function data_update_page( $tab ) {
        if ( $tab == 'data_update' ) {
            ...
            if ( empty( $act ) ) {
                wp_die( __('No action specified', 'follow_up_emails' ) );
            }
            $return = admin_url( 'admin.php?page=followup-emails' );
            if ( !empty( $_GET['ref'] ) ) {
                $return = admin_url( $_GET['ref'] );
            }
            if ( $act == 'sum_orders' ) {
                $args = array(
    ...
    public function scan_customer_orders() {
        ...
        wp_redirect( 'admin.php?page=followup-emails&tab=order_import');
        exit;
    }
    ...
    public function add_import_action_link( $email ) {
        if ( $email->import_order_flag == 1 ) {
            echo '| <small><a href="'. esc_url( $this->get_email_import_url( $email->id )) .'" class="email-import-orders" data-id="'. $email->id .'">'. __('Import Orders', 'follow_up_emails') .'</a></small>';
        }
    }
    ...
    public function addon_save_settings() {
        ...
        if ( $post['section'] == 'integration' ) {
            ...
            $conversion = (isset($_POST['wc_conversion_days'])) ? absint($_POST['wc_conversion_days']) : 14;
            update_option( 'fue_wc_conversion_days', $conversion );

            $abandoned_cart_value   = (isset($_POST['wc_abandoned_cart_value'])) ? absint($_POST['wc_abandoned_cart_value']) : 3;
            $abandoned_cart_unit    = (isset($_POST['wc_abandoned_cart_unit'])) ? $_POST['wc_abandoned_cart_unit'] : 'hours';
     ...
     */
    public function addon_save_subscribers_settings() {
        // checkout subscription
        $enable_checkout_subscription   = (isset($_POST['enable_checkout_subscription'])) ? (int)$_POST['enable_checkout_subscription'] : 0;
        $enable_account_subscription    = (isset($_POST['enable_account_subscription'])) ? (int)$_POST['enable_account_subscription'] : 0;

        $checkout_label  = $_POST['checkout_subscription_field_label'];
        $checkout_list   = $_POST['checkout_subscription_list'];
        $checkout_default= $_POST['checkout_subscription_default'];
    ...
    public function storewide_variables( $email ) {
        ...
        <li class=""><strong>{item_prices}</strong> <img class="help_tip" title="<?php _e('Displays a list of purchased items, quantities, and prices.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
        <li class=""><strong>{item_prices_image}</strong> <img class="help_tip" title="<?php _e('Displays a list of purchased items, quantities, and prices with thumbnails.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
        <li class=""><strong>{item_codes_prices}</strong> <img class="help_tip" title="<?php _e('Displays a list of the purchased items with their quantities, codes, and prices.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
        <li class=""><strong>{item_prices_categories}</strong> <img class="help_tip" title="<?php _e('Displays a list of the purchased items, with their quantities, prices, and categories.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
    ...
    public function product_variables( $email ) {
        ...
        ?>
        <li class="var hideable"><strong>{download_url}</strong>  <img class="help_tip" title="<?php _e('The URL of the downloadable file.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
        <li class="var hideable"><strong>{download_filename}</strong>  <img class="help_tip" title="<?php _e('The name of the downloadable file.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
        ...
        <li class=""><strong>{item_code}</strong> <img class="help_tip" title="<?php _e('The code number of the purchased item.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
        <li class=""><strong>{item_url}</strong> <img class="help_tip" title="<?php _e('The URL of the purchased item.', 'follow_up_emails'); ?>" src="<?php echo FUE_TEMPLATES_URL; ?>/images/help.png" width="16" height="16" /></li>
    ...
    public function filter_orders_by_email() {
        ...
        if ( ! empty( $emails ) ) {
            ...
            <select name="_email_id" id="dropdown_emails_used">
                <option value=""><?php _e( 'Filter by emails sent/queued', 'follow_up_emails' ); ?></option>
    ...
    public function add_filterable_where( $where ) {
        ...
        if ( ! empty( $_GET['_email_id'] ) ) {
            ...
            $where .= $wpdb->prepare( " AND eo.email_id=%d", wc_clean( $_GET['_email_id'] ) );
        }
