...
class FUE_Email {
    ...
    public function get_type() {
        ...
        if ( $this->type == 'generic' || $this->type == 'normal' || $this->type == 'product' ) {
    ...
    /**
     * Check if this email can still be edited.
     *
     * Some of the email's data cannot be edited once it's been sent or in the queue
     *
     * @return bool
     */
    public function content_editable() {
        $editable   = true;
        ...
        return apply_filters( 'fue_email_content_editable', $editable, $this );
    ...
    public function get_trigger_string() {
        ...
        if ( $this->type == 'manual' ) {
            ...
            if ( $this->duration == 'date' ) {
                ...
                $trigger = sprintf(
                    __('%d %s after user signs up', 'follow_up_emails'),
                    $this->interval,
    ...
    public function update_status( $new_status ) {
        ...
        FUE_Followup_Logger::log( $this->id, sprintf(__('<p>Email attributes updated:</p><p><strong>status</strong> from <em>%s</em> to <em>%s</em></p>', 'follow_up_emails'), $old_status, $new_status ) );
    }
    ...
    public function apply_template( $message = null ) {
        ...
        foreach ( $sections as $section ) {
            ...
            if ( !empty( $section_body ) ) {
                $contents = str_replace( '{section:'. $section .'}', wpautop( $section_body[0] ), $contents );
            }
        }
