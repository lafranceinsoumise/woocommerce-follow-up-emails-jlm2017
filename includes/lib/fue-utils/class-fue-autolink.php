...
class FUE_Autolink {
    ...
    public static function link($text, $limit=0, $tagfill='', $auto_title = true){
        $text = self::autolink_do($text, '![a-z][a-z-]+://!i',	$limit, $tagfill, $auto_title);
        $text = self::autolink_do($text, '!(mailto|skype):!i',	$limit, $tagfill, $auto_title);
        $text = self::autolink_do($text, '!www\\.!i',			$limit, $tagfill, $auto_title, 'http://');
        return $text;
    }

    public static function autolink_do($text, $sub, $limit, $tagfill, $auto_title, $force_prefix=null){
        ...
        while (($cursor < strlen($text)) && $loop){
            ...
            if ($ok){
                if (preg_match('/^([a-z0-9\-\.\/\-_%~!?=,:;&+*#@\(\)\$]+)/i', $post, $matches)){
                    ...
                    $display_url = self::autolink_label($display_url, $limit);
                    ...
                    $link_url = apply_filters( 'fue_autolink_url', $link_url );
                    $display_url = apply_filters( 'fue_autolink_display', $display_url );
    ...
    public static function autolink_label($text, $limit){
    ...
    public static function autolink_email($text, $tagfill=''){
