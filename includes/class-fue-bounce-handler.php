...
class FUE_Bounce_Handler {
    ...
    public function schedule_bounce_handling() {
        if ( wc_next_scheduled_action( 'fue_bounce_handler', null, 'fue' ) ) {
            return;
        }

        wc_schedule_recurring_action( current_time( 'timestamp', true ), 300, 'fue_bounce_handler', array(), 'fue' );
    }

    public function unschedule_bounce_handling() {
        wc_unschedule_action( 'fue_bounce_handler', array(), 'fue' );
    }
     ...
     *
     * @param POP3 $pop3 Resource must already have an active connection
     */
    ...
    public function bounce( $fue_email, $queue_item, $hard = false ) {
        ...
        if ( $bounce_attempts == 1 || $made_attempts > $bounce_attempts ) {
