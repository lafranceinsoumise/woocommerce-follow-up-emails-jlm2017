...
function fue_save_email( $args ) {
    ...
    if ( !empty( $_POST['post_title'] ) ) {
        $args['prev_name'] = $_POST['post_title'];
    }

    if ( !empty( $_POST['post_excerpt'] ) ) {
        $args['prev_subject'] = $_POST['post_excerpt'];
    }

    if ( !empty( $_POST['content'] ) ) {
        $args['prev_message'] = $_POST['content'];
    }
...
function fue_add_subscriber( $email, $list = '' ) {
    return Follow_Up_Emails::instance()->newsletter->add_subscriber( $email, $list );
}
