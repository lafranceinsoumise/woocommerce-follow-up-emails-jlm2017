...
class FUE_Reports_Customers_Table extends WP_List_Table {
    ...
    public function prepare_items() {
        ...
        if ( !empty($_GET['_customer_user']) ) {
            ...
            if ( is_email( $_GET['_customer_user'] ) ) {
                $sql .= " AND c.email_address = '". esc_sql( sanitize_email( $_GET['_customer_user'] ) ) ."'";
            } else {
               $sql .= " AND u.ID = ". esc_sql( absint( $_GET['_customer_user'] ) );
            }
        }
    ...
    function extra_tablenav( $which ) {
        if ( 'top' == $which ) {
            ...
            if ( ! empty( $_GET['_customer_user'] ) ) {
                if ( is_email( $_GET['_customer_user'] ) ) {
                    $user_id     = 0;
                    $user_string = esc_html( sanitize_email( $_GET['_customer_user'] ) .' (Guest)' );
                } else {
            ...
            <input
                ...
                data-placeholder="<?php _e('Search for a customer...', 'follow_up_emails'); ?>"
                name="_customer_user"
...
<div class="wrap">
    <h2><?php _e('Customer Data Manager', 'follow_up_emails'); ?></h2>
