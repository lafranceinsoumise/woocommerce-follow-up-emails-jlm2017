<div class="options_group">
    ...
    <p class="form-field">
        <label><?php _e('Sending Delay', 'follow_up_emails'); ?></label>
        ...
        <select name="interval_duration" id="interval_duration" class="interval_duration hideable">
            ...
            foreach ( $durations as $key => $duration ):
                ...
                ?>
                <option class="interval_duration_<?php echo $key; ?> hideable" value="<?php echo esc_attr($key); ?>" <?php echo $selected; ?>><?php echo Follow_Up_Emails::get_duration( $key, $email->interval ); ?></option>
            <?php endforeach; ?>
        </select>
    ...
    <p class="form-field">
        <label><?php _e('Trigger', 'follow_up_emails'); ?></label>
        ...
        <span class="hide-if-date interval_type_span hideable">
            ...
            <select name="interval_type" id="interval_type" class="interval_type hideable">
                ...
                foreach ( $triggers as $key => $value ):
                    ...
                    ?>
                    <option
                        class="interval_type_option interval_type_<?php echo $email_type->id; ?> interval_type_<?php echo $key; ?> <?php if ( $key != 'purchase' && $key != 'completed' ) echo 'non-reminder'; ?> hideable <?php do_action('fue_form_interval_type', $key); ?>"
        ...
        <span class="show-if-date interval_date_span hideable">
            ...
            <select name="send_date_minute">
                ...
                for ( $x = 0; $x <= 55; $x+=5 ):
                    ?>
                    <option value="<?php echo $x; ?>" <?php selected( $email->send_date_minute, $x ) ?>><?php echo $x; ?></option>
                <?php endfor; ?>
            </select>
            <?php
    ...
    <p class="form-field hideable show-if-list_signup">
        <label><?php _e('List', 'follow_up_emails'); ?></label>

        <?php
        $selected_list = (empty( $email->meta['list'] ) ) ? 'any' : $email->meta['list'];
        ?>
        <select name="meta[list]">
            <option value="any" <?php selected( $selected_list, 'any' ); ?>><?php _e('Any list', 'follow_up_emails'); ?></option>
            <?php foreach ( Follow_Up_Emails::instance()->newsletter->get_lists() as $list ): ?>
                <option value="<?php echo $list['id']; ?>" <?php selected( $selected_list, $list['id'] ); ?>><?php echo $list['list_name']; ?></option>
            <?php endforeach; ?>
        </select>
    </p>
    ...
    <div id="trigger_conditions" class="hide-if-date non-signup">
        ...
        if ( $email_conditions ):
            ...
            foreach ( $email_conditions as $idx => $email_condition ):
                ...
                ?>
                <fieldset id="condition_<?php echo $idx; ?>">

                    <?php _e('AND', 'follow_up_emails'); ?>

                    <select name="conditions[<?php echo $idx; ?>][condition]" class="condition">
                        <?php
        ...
        <p>
            <a class="button button-primary btn-add-condition"><?php _e('Add Additional Rule', 'follow_up_emails'); ?></a>
        </p>
    </div>
