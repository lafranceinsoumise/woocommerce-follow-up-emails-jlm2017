<div class="options_group storewide non-signup hideable <?php do_action('fue_form_excluded_category_tr_class', $email); ?> excluded_category_tr">
    ...
    <p class="form-field">
        ...
        <select id="excluded_category_ids" name="meta[excluded_categories][]" class="select2" data-placeholder="<?php _e('Select categories&hellip;', 'follow_up_emails'); ?>"  multiple style="width:400px;">
            <option></option>
            <?php
            ...
            foreach ($categories as $category):
                ?>
                <option value="<?php _e($category->term_id); ?>" <?php selected( in_array($category->term_id, (array)$excluded), true ); ?>><?php echo esc_html($category->name); ?></option>
            <?php endforeach; ?>
        </select>
    </p>
